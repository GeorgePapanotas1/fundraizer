  ...
  <form @submit.prevent="onSubmit" class="space-y-6">
    <UiCard class="card-section space-y-4">
      <div class="grid md:grid-cols-2 gap-4">
        ...
        <UiInput id="cover_image" label="Cover image URL" v-model="form.coverImage" placeholder="https://.../cover.jpg" />
      </div>
      <UiTextarea id="short_desc" label="Short description" v-model="form.shortDescription" placeholder="A one‑sentence summary shown in listings" />
      <div class="grid md:grid-cols-3 gap-4">
        <UiSelect id="category" label="Category" v-model="form.category">
          <option value="">Select a category</option>
          <option v-for="c in categories" :key="c.id" :value="String(c.id)">{{ c.name }}</option>
          <!-- Fallbacks if categories not provided -->
      ...
      <UiTextarea id="long_desc" label="Long description" v-model="form.longDescription" placeholder="Tell the full story, goals, and impact…" rows="6" />
      <div class="grid md:grid-cols-2 gap-4">
        <UiInput id="location" label="Location" v-model="form.location" placeholder="Global / EMEA / Nairobi…" />
        <UiInput id="tags" label="Tags (comma separated)" v-model="form.tags" placeholder="trees,volunteering,environment" />
      </div>
    ...
    <div class="flex items-center gap-3">
      ...
      <UiButton v-if="mode === 'edit'" type="button" variant="danger" :disabled="submitting">Archive campaign</UiButton>
    </div>
  </form>
</template>
...
export interface CampaignFormValues {
  ...
  category: string; // holds category id (ulid or numeric) as string
  goal: string | number | null;
  startDate: string | null;
  endDate: string | null;
  status: string; // holds enum value like 'draft' | 'pending_approval' | 'active' | 'closed'
  coverImage: string;
...
const props = withDefaults(defineProps<{ mode?: Mode; initialValues?: Partial<CampaignFormValues>; submitting?: boolean; categories?: CategoryOption[]; statuses?: StatusOption[] }>(), {
  mode: 'create',
  initialValues: () => ({}),
  submitting: false,
  categories: () => [],
